...
class gringarten:
    ...
    #                     proppantconductivity=100*darcy,backpressure=5*1e6):
    ...
    def EGS_demo(self,numfracs=50,fracspacing=50,fracaperture=0.001,
            ...
            proppantconductivity=50*darcy,backpressure=1*1e6):
        L = wellspacing
        W = wellspacing
        mu = 0.1*cP
        g = 9.81
        ...
        for q in range(0,len(Qs)):
            ...
            ax2.plot([1,2,3,4],[P1*1e-6,(P2+staticpressure)*1e-6,(P3+staticpressure)*1e-6,P4*1e-6],label='test %i: %.3f m3/s' %(q, Qs[q]))
        ...
        ax2.plot([1,4],[openingpressure*1e-6,openingpressure*1e-6],'--',label='max allowable at P2')
    ...
    def basic(self,rocktemp=473.15,rockKt=2.306,rockSv=2133,
              flowsplit=2,wellspacing=110,numfracs=50,fracspacing=50,
        ...
        T_mixed = np.zeros(len(timepassed))
        for f in range(0,len(q_fracs)):
            self.Tvt(fracspacing=fracspacing,Qinj=q_fracs[f]/flowsplit,

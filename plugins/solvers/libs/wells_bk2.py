...
def gen_wells(s=iogt.setup()):
    ...
    cLen = pLen - iLen
    #segments
    segle = s.Well_TargetDepth_m/s.Well_SegmentCount_units #segment length, m
    ...
    # ************************************************************************
        ...
        p0s = []
        for i in range(0,num):
            if (i == 0) and (s.Well_InjectorCount_wells > 0):
                #first production well
                p0s += [i0]
                #first injection well
                # i0s += [sg.rotatePoints([p0],vInj,(np.pi - phase))[0]]
                i0s += [sg.rotatePoints([p0],vInj,phase)[0]]
            elif (i == 0):
                #first production well
                p0s += [i0]
            elif (i == 1):
                #skip second injector
                p0s += [p0s[-1] + 2.0*vRht*s.Well_Spacing_m*np.cos(phase)]
            elif (i > 1) and (s.Well_InjectorCount_wells > 0):
                #place additional wells in pairs
                i0s += [i0s[-1] + 2.0*vRht*s.Well_Spacing_m*np.cos(phase)]
                p0s += [p0s[-1] + 2.0*vRht*s.Well_Spacing_m*np.cos(phase)]
            else:
                #keep adding producers
                p0s += [p0s[-1] + 2.0*vRht*s.Well_Spacing_m*np.cos(phase)]
        #recenter
        c0 = np.zeros(3)
        if num < 2:
        ...
        for i in range(0,len(i0s)):
            ...
            iss += [np.asarray([i1s[i][0],i1s[i][1],s.Well_TargetDepth_m+0.1])]
        #production well segments
        p1s = [] #startpoints
        p2s = [] #endpoints
        pss = [] #surface segments
        ...
        pid = np.asarray(range(0,len(p0s)))+n+len(i0s)
        for i in range(0,len(i0s)):
            azn, dip, dis = sg.azn_dip(iss[i]+h,iss[i])
            wells += [line(iss[i][0]+h[0],iss[i][1]+h[1],iss[i][2]+h[2],dis,azn,dip,'injector',sra,srb,src,rgh,iid[i])]
        for i in range(0,len(p0s)):
            azn, dip, dis = sg.azn_dip(pss[i]+h,pss[i])
            wells += [line(pss[i][0]+h[0],pss[i][1]+h[1],pss[i][2]+h[2],dis,azn,dip,'producer',sra,srb,src,rgh,pid[i])]
        #place surface segments
        for i in range(0,len(i0s)):
            azn, dip, dis = sg.azn_dip(iss[i],i1s[i])
            wells += [line(iss[i][0],iss[i][1],iss[i][2],dis,azn,dip,'pipe',ira,irb,irc,rgh,iid[i])]
        for i in range(0,len(p0s)):
            azn, dip, dis = sg.azn_dip(pss[i],p1s[i])
            wells += [line(pss[i][0],pss[i][1],pss[i][2],dis,azn,dip,'pipe',ira,irb,irc,rgh,pid[i])]
        #place injection wells
